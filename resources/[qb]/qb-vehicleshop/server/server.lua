...
AddEventHandler('qb-vehicleshop.CheckMoneyForVeh', function(veh, price, name, vehicleProps)
    ...
    if xPlayer.PlayerData.money['bank'] >= tonumber(price) or xPlayer.PlayerData.money['cash'] >= tonumber(price) then
        if xPlayer.PlayerData.money['cash'] >= tonumber(price) then 
            xPlayer.Functions.RemoveMoney('cash', tonumber(price))
        elseif xPlayer.PlayerData.money['bank'] >= tonumber(price) then
